select
		...
		,cpuTime					    = case 
											          when a.cpu_time >= 86400000 then cast(a.cpu_time / 86400000 as varchar(10)) + 'd ' + cast(CONVERT(varchar, DATEADD(ms, a.cpu_time, 0), 108) as varchar(10))
										            else cast(CONVERT(varchar, DATEADD(ms, a.cpu_time, 0), 108) as varchar(10))
									          end
		,waitTime					    = case 
                                when a.wait_time >= 86400000 then cast(a.wait_time / 86400000 as varchar(10)) + 'd ' + cast(CONVERT(varchar, DATEADD(ms, a.wait_time, 0), 108) as varchar(10))
                                else cast(CONVERT(varchar, DATEADD(ms, a.wait_time, 0), 108) as varchar(10))
                            end
		,runTime					    = case 
                                when a.total_elapsed_time >= 86400000 then cast(a.total_elapsed_time / 86400000 as varchar(10)) + 'd ' + cast(CONVERT(varchar, DATEADD(ms, a.total_elapsed_time, 0), 108) as varchar(10))
